...
public class DynamicDataDemo extends ApplicationFrame implements ActionListener {
	...
	/** The time interval of the timer used to add new data. */
	private int interval = 1;

	/** The timer used to add new data. */
	private Timer timer;
			...
			if (timer == null) {
				timer = new Timer(interval, new ActionListener() {
					...
					public void actionPerformed(ActionEvent e) {
						final double factor = 0.90 + 0.2 * Math.random();
						DynamicDataDemo.this.lastValue = DynamicDataDemo.this.lastValue
								* factor;
						final Millisecond now = new Millisecond();
						System.out.println("Now = " + now.toString());
						DynamicDataDemo.this.series.add(new Millisecond(),
								DynamicDataDemo.this.lastValue);
					}
